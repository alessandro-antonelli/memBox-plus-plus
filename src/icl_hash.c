...
pthread_mutex_t nentriesMutex = PTHREAD_MUTEX_INITIALIZER;
//Aggiunta del candidato - Alessandro Antonelli matricola 507264 - Necessario per accedere in mutua esclusione all'unico campo della tabella hash che la necessita (in quanto può venire letto e scritto da più thread), e cioè nentries.
...
icl_hash_insert(icl_hash_t *ht, void* key, void *data)
{
	...
	MyLock(nentriesMutex);
    ht->nentries++;
	MyUnlock(nentriesMutex);
...
icl_hash_update_insert(icl_hash_t *ht, void* key, void *data, void **olddata)
{
	...
	/*	Le due righe seguenti sono una mia aggiunta per evitare che la variabile di tipo icl_entry_t,
		allocata dala icl_hash_insert, non venga persa (memory leak).
		La modifica è stata concordata con il prof. Torquati nell'email del 25/9/16.
		Alessandro Antonelli - UniPi - matricola 507264		*/
	    free(curr);
	    break;
        }
...
/*	La rimozione delle seguenti due righe è opera mia ed è necessaria per rendere
	l'implementazione coerente con la specifica: infatti in olddata deve essere
	inserito un puntatore ai dati dell'elemento prima della loro sostituzione,
	mentre queste due righe (inspiegabilmente) facevano sì che il contenuto del
	puntatore fosse	sempre NULL.
	La modifica è stata concordata con il prof. Torquati nell'email del 25/9/16.
		Alessandro Antonelli - UniPi - matricola 507264
...
/*
L'implementazione della funzione è stata modificata dal candidato (Alessandro Antonelli, mat. 507264) per renderla conforme a quanto richiesto nelle specifiche di membox++ (stampa in un file binario e non testuale, uso delle syscall write/read in luogo delle chiamate di libreria standard fprintf/fscanf)
*/

int
icl_hash_dump(int dump, icl_hash_t* ht)
{
    ...
    for(i=0; i<ht->nbuckets; i++) {
        ...
        for(curr=bucket; curr!=NULL; ) {
            if(curr->key)
	    {
		hashTableEntry* elem = (hashTableEntry*) curr->data;

		//Stampo chiave e size
		CheckMenoUno(write(dump, (membox_key_t*)curr->key, sizeof(membox_key_t)) != sizeof(membox_key_t), TRUE, "scrittura del dump - chiave oggetto");
		CheckMenoUno(write(dump, &(elem->len), sizeof(unsigned int)) != sizeof(unsigned int), TRUE, "scrittura del dump - size oggetto");

		//Stampo contenuto
		CheckMenoUno(write(dump, elem->data, elem->len) != elem->len, TRUE, "scrittura del dump - contenuto oggetto");
	    }
            curr=curr->next;
        }
    }
